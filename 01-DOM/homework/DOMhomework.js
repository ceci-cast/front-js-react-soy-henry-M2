...
// Tu código acá:
  ...
  span.innerHTML += 'Cecilia Castellano';
...
function buildToDo(todo, index) {  // { description: '', complete: false }
  ...
  let toDoText = document.createElement('span');  // <span id = 1 > Hola </span>
...
function addToDo() {
 ...
 if (input.value !== '') {
  let todo = new ToDo (input.value); // {description: 'hola', complete: false}
  toDoItems.push(todo);
  input.value = '';
  displayToDos();
 }
